...
- MMU由一种称为分段单元的硬件电路把一个逻辑地址转换为线性地址，再由分页单元的硬件电路把线性地址转换为物理地址。
- 逻辑地址：一个16位段选择符（高13位相对地址）和32位偏移量（4G寻址）
- 线性地址：32位无符号数，表示4G的地址（0x00000000-0xffffffff）、（目录高10位、页表中间10位、偏移量低12位）
- 物理地址：32位或36位无符号整数
- 内存仲裁器（memory arbiter）：存在于总线和每个RAM直线；DMA和CPU的并发操作，CPU和CPU的并发操作
...
- 段寄存器：cs（代码段、特权级0内核态3用户态）、ss（栈段）、ds（数据段）、es、fs、gs；
  - 存放段选择符
  - 根据特权级选择内核或用户
- 段描述符：==8字节长==、存放在全局描述符表GDT（Global Descriptor Table）、局部描述符表LDT（Local Descriptor Table）、GDT放在gdtr控制寄存器、LDT存放在ldtr寄存器中。
  - GDT第一项总为0，能够存放最多8191（2^13 - 1）个段描述符，每个CPU对应一个GDT，cpu_gdt_table数组
  - 段有三种存取权限（读、写、执行）
  - 代码段描述符：用户代码段描述符、内核代码段描述符
  - 数据段描述符：用户数据段描述符、内核数据段描述符
  - 任务状态段描述符TSSD（Task State Segment Descriptor）：每个CPU有一个任务状态段，init_tss数组
  - 系统段描述符
  - 局部描述符表描述符LDTD：依赖于正在执行的进程
  - 局部线程存储段描述符TLSD：依赖于正在执行的进程
- ==段选择符装入段寄存器，仅第一次访问GDT或LDT获得对应段描述符，之后只有当段寄存器改变才需要重新查找。==
- 逻辑地址->线性地址：根据段选择符TI选择GDT或LDT，根据选择的gdtr或ldtr获得线性基地址，加上段选择符13位index乘以==8==，最后再加上逻辑地址偏移量。
...
- 同步和互斥（信号、互斥锁、读写锁、自旋锁、条件变量、信号量）
  ...
  - ==自旋锁==上锁受阻时线程不阻塞而是在循环中轮询查看能否获得该锁，没有线程的切换因而没有切换开销，不过对 CPU 的霸占会导致 CPU 资源的浪费。 所以自旋锁适用于并行结构（多个处理器）或者适用于锁被持有时间短而不希望在线程切换产生开销的情况。 
    ...
    ![image-20240913191445550](C:/Users/Administrator/AppData/Roaming/Typora/typora-user-images/image-20240913191445550.png)

    ![image-20240913191549111](C:/Users/Administrator/AppData/Roaming/Typora/typora-user-images/image-20240913191549111.png)
...
- 与进程地址空间有关的全部信息都包含在内存描述符mm_struct中，用一个双向链表存放
- 内核线程的内存描述符
- 内存描述符存放在红黑树中
- 线性区对象：进程所拥有的所有线性区，由一个简单链表链接在一起，区分线性区和页之间的关系（每个线性区都由一组号码连续的页组成）
- 堆：特殊的线性区，用于满足进程的动态内存请求（内存描述符中start_brk）

### ==I/O管理==

#### 概念

- I/O端口（可以映射到物理地址空间）、用resource记录分配给每个硬件设备的I/O端口
- I/O接口：I/O端口和对应设备控制器之间的硬件电路
- 设备控制器
...
- sysfs文件系统
  - kobject数据结构、kset、subsystem
- proc文件系统
- 组件：设备device对象、驱动程序device_driver对象、总线bus_type、类class对象（input子系统类）
  - ![image-20240914155651407](C:/Users/Administrator/AppData/Roaming/Typora/typora-user-images/image-20240914155651407.png)
- 设备文件：I/O设备可以把它当成设备文件
  - 块设备、字符设备、网卡
  - 设备标识符：主设备号、次设备号
  - mknod创建设备文件、
- 设备驱动程序
  - 内核例程的集合、使得硬件设备相应控制设备的编程接口，接口是规范的VFS函数集
  - 注册设备驱动程序：分配device_driver描述符
- 字符设备驱动程序
- 块设备驱动程序
  - 页、段、块缓冲区、扇区、通用块层bio
...
- 文件系统组成
  - <strong>超级块</strong>super_block：存放文件系统本身的信息，比如记录了每个区域的大小，或未被使用的磁盘块的信息。（不同版本稍有差别）
  - <strong>i-节点表</strong>inode：每个文件都有其属性，大小，最近修改时间等等，这些被存储在ino_t 的结构体中，所有的i-节点都有一样的大小，i-节点表就是这样一些节点的列表。（表中的每个i-节点都通过位置来标志，例如标志为2的i-节点位于文件系统i-节点表中的第3个位置 ）
  - <strong>文件对象</strong>file：描述进程怎么样与一个打开的文件进行交互
  - <strong>目录项对象</strong>dentry：描述进程怎么样与一个打开的文件进行交互
    - 目录项高速缓存：软件缓存方式
...
- 安装/卸载普通文件系统、安装/卸载根文件系统（rootfs+实际根文件系统）
